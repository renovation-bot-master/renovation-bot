...
SIZE_INPUT, ADDRESS_INPUT = range(4, 6)
...
async def confirm_services(query, context):
    ...
    context.user_data['state'] = 'waiting_size'

async def input_size(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.user_data.get('state') != 'waiting_size':
        return
...
def main():
    ...
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, input_size))
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, input_address))

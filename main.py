...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('setup_step') == 'name':
        ...
        msg = await context.bot.send_message(chat_id=update.effective_chat.id, text="📝 <b>Шаг 2 из 5</b>\n\nТелефон (8ххххххххх):", parse_mode="HTML", reply_markup=get_menu_buttons())
        ...
        msg = await context.bot.send_message(chat_id=update.effective_chat.id, text="📝 <b>Шаг 3 из 5</b>\n\nВК (или 'пропустить'):", parse_mode="HTML", reply_markup=get_menu_buttons())
        context.user_data['last_msg_id'] = msg.message_id
        ...
        msg = await context.bot.send_message(chat_id=update.effective_chat.id, text="📝 <b>Шаг 4 из 5</b>\n\nTelegram (или 'пропустить'):", parse_mode="HTML", reply_markup=get_menu_buttons())
        context.user_data['last_msg_id'] = msg.message_id
        ...
        msg_text = "✅ <b>Профиль готов!</b>\n\n"
        msg_text += f"Название: <b>{profile['name']}</b>\n"
        msg_text += f"Телефон: <b>{profile['phone']}</b>\n"
        msg_text += f"ВК: {profile.get('vk', '—')}\n"
        msg_text += f"Telegram: {profile.get('telegram', '—')}"
        msg = await context.bot.send_message(chat_id=update.effective_chat.id, text=msg_text, parse_mode="HTML", reply_markup=get_menu_buttons())
        context.user_data['last_msg_id'] = msg.message_id
        ...
        msg = await context.bot.send_message(chat_id=update.effective_chat.id, text=f"✅ {field} обновлен!", reply_markup=get_menu_buttons())
        context.user_data['last_msg_id'] = msg.message_id
        ...
        try:
            parts = text.split()
            if len(parts) != 3:
                msg = await context.bot.send_message(chat_id=update.effective_chat.id, text="❌ Ошибка! Формат: 5 4 3", reply_markup=get_menu_buttons())
                context.user_data['last_msg_id'] = msg.message_id
                return
            ...
            msg_text = f"✅ Размеры: {d}×{sh}×{v}м\n📐 Площадь: {area:.2f}м²\n\n📍 Адрес объекта:"
            msg = await context.bot.send_message(chat_id=update.effective_chat.id, text=msg_text, reply_markup=get_menu_buttons())
            context.user_data['last_msg_id'] = msg.message_id
        except:
            msg = await context.bot.send_message(chat_id=update.effective_chat.id, text="❌ Ошибка! Введите числа", reply_markup=get_menu_buttons())
            context.user_data['last_msg_id'] = msg.message_id
...
async def generate_estimate_pdf(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await context.bot.send_message(chat_id=update.effective_chat.id, text="🎉 Готово!", reply_markup=get_menu_buttons())
    context.user_data['last_msg_id'] = msg.message_id

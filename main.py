...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if profile.get('name'):
        await update.message.reply_text("👋 Добро пожаловать обратно!", reply_markup=get_menu_buttons())
    else:
        text = "👋 <b>Добро пожаловать!</b>\n\nБот для составления смет на ремонт.\n\n✅ Выбор услуг\n✅ Расчёт стоимости\n✅ PDF смета\n✅ История\n\n<b>Создадим профиль!</b>"
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('setup_step') == 'name':
        ...
        if context.user_data.get('last_msg_id'):
            await context.bot.edit_message_text(chat_id=update.effective_chat.id, message_id=context.user_data['last_msg_id'], text="📝 <b>Шаг 2 из 5</b>\n\nТелефон (8ххххххххх):", parse_mode="HTML", reply_markup=get_menu_buttons())
        else:
        ...
        if context.user_data.get('last_msg_id'):
            await context.bot.edit_message_text(chat_id=update.effective_chat.id, message_id=context.user_data['last_msg_id'], text="📝 <b>Шаг 3 из 5</b>\n\nВК (или 'пропустить'):", parse_mode="HTML", reply_markup=get_menu_buttons())
        else:
        ...
        if context.user_data.get('last_msg_id'):
            await context.bot.edit_message_text(chat_id=update.effective_chat.id, message_id=context.user_data['last_msg_id'], text="📝 <b>Шаг 4 из 5</b>\n\nTelegram (или 'пропустить'):", parse_mode="HTML", reply_markup=get_menu_buttons())
        else:
        ...
        if context.user_data.get('last_msg_id'):
            await context.bot.edit_message_text(chat_id=update.effective_chat.id, message_id=context.user_data['last_msg_id'], text=msg, parse_mode="HTML", reply_markup=get_menu_buttons())
        else:
        ...
        if context.user_data.get('last_msg_id'):
            await context.bot.edit_message_text(chat_id=update.effective_chat.id, message_id=context.user_data['last_msg_id'], text=f"✅ {field} обновлен!", reply_markup=get_menu_buttons())
        else:
        ...
        try:
            ...
            if len(parts) != 3:
                if context.user_data.get('last_msg_id'):
                    await context.bot.edit_message_text(chat_id=update.effective_chat.id, message_id=context.user_data['last_msg_id'], text="❌ Ошибка! Формат: 5 4 3", reply_markup=get_menu_buttons())
                else:
            ...
            if context.user_data.get('last_msg_id'):
                await context.bot.edit_message_text(chat_id=update.effective_chat.id, message_id=context.user_data['last_msg_id'], text=msg, reply_markup=get_menu_buttons())
            else:
                await update.message.reply_text(msg, reply_markup=get_menu_buttons())
        except:
            if context.user_data.get('last_msg_id'):
                await context.bot.edit_message_text(chat_id=update.effective_chat.id, message_id=context.user_data['last_msg_id'], text="❌ Ошибка! Введите числа", reply_markup=get_menu_buttons())
            else:
...
async def generate_estimate_pdf(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🎉 Готово!", reply_markup=get_menu_buttons())
